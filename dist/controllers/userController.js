...
export async function register(req, res) {
  ...
  try {
    let nuevoUsuario = await Test.create({
      ...
      fields: ['nombre', 'pass', 'mail', 'rol', 'puntaje', 'nivel', 'foto', 'cantEnvios', 'redsocial', 'uidfirebase', 'token']
    });
...
export async function login(req, res) {
  ...
  if (idToken != -1) {
    ...
    admin.auth().verifyIdToken(idToken).then(async function (decodedToken) {
      ...
      let uid = decodedToken.uid; //.....Busco el uid en mi base de datos............
      ...
      try {
        const userFound = await Test.findOne({
          ...
          attributes: ['id', 'nombre', 'pass', 'mail', 'rol', 'puntaje', 'nivel', 'foto', 'cantEnvios', 'redsocial', 'uidfirebase', 'token']
        });
...
export async function getDeliverysPorUsuario(req, res) {
  ...
  try {
    var sqlquery = 'select Distinct(users.*) from users, pedidos where pedidos.ped_deliveryid=users.id and pedidos.ped_userid='.concat(iduser).concat(' ORDER BY users.nombre '); //res.json(sqlquery);

    var usuarios = await sequelize.query(sqlquery, {
